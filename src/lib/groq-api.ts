...
export async function sendMessageToGroq(message: string): Promise<string> {
  try {
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'mixtral-8x7b-32768', // Using Mixtral model instead of the one in your example
        messages: [
          {
            role: 'system',
            content: 'You are Baymax, a personal healthcare companion from Big Hero 6. You are caring, gentle, and focused on helping with health and wellness. Always maintain a warm, helpful, and slightly formal tone like the character.'
          },
          {
            role: 'user',
            content: message
    ...
    return data.choices[0]?.message?.content || "I'm here to help you. Could you please rephrase your question?";
